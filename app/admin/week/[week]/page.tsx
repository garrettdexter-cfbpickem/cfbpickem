...
export default async function AdminWeekPage({ params }: { params: { week: string } }) {
    ...
    const gamesForWeek = await getGamesForWeek(week);

  // Checked (included_in_pickem) games float to the top so the admin doesn't
  // have to scroll through a full slate to find what's already selected;
  // within each group, keep the original kickoff-time order.
  const games = [...gamesForWeek].sort((a, b) => {
        if (a.included_in_pickem !== b.included_in_pickem) {
                return a.included_in_pickem ? -1 : 1;
        }
