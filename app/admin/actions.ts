...
import { syncWeek, lockLinesForWeek, scoreWeek } from "@/lib/sync";
...
// Manual override for the Sunday 4am cron. Pulls current scores/status from
// CFBD for every game in this week and (re)computes ats_result for any game
// that's final and has a locked spread. Safe to click as many times as you
// want, at any time - e.g. if the cron ran before some late games finished,
// or you just want to force an update right now instead of waiting.
export async function scoreWeekAction(week: number): Promise<void> {
    await scoreWeek(week);
    revalidatePath(`/admin/week/${week}`);
    revalidatePath(`/week/${week}`);
    revalidatePath("/");
