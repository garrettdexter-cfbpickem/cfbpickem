...
export async function getAllGames(): Promise<Game[]> {
      ...
      const { data, error } = await supabase
        ...
        // Supabase/PostgREST silently caps any query at 1000 rows unless a
    // higher limit is explicitly requested. Once the "games" table passed
    // 1000 total rows (easy to do once several weeks are synced), this
    // function was quietly truncating to the first 1000 rows in week order -
    // meaning later weeks (and the picks referencing their games) simply
    // never showed up in getCurrentWeek() or the standings calculation, with
    // no error of any kind. 10000 is comfortably above anything this app
    // will realistically accumulate in a season.
    .limit(10000);
...
export async function getGamesForWeek(week: number): Promise<Game[]> {
      ...
      const { data, error } = await supabase
        ...
        .eq("week", week)
        .order("kickoff_time", { ascending: true })
        // Defensive: same 1000-row default cap as getAllGames. A single week is
    // unlikely to ever hit this, but it's cheap insurance.
    .limit(10000);
...
export async function getAllPicks(): Promise<Pick[]> {
      const supabase = supabaseAdmin();
      const { data, error } = await supabase.from("picks").select("*").limit(10000);
