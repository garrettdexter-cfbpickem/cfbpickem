 ...
 * games. Only games that are still included_in_pickem, 'final', and have a
 * non-null ats_result count, and only picks matching the covering team score
 * a point. Pushes and incorrect picks score 0.
 *
 * The included_in_pickem check matters: if an admin ever unchecks a game from
 * a week's pool after players already picked it (e.g. to correct a mistake),
 * the pick row stays in the database and the game can still get scored by
 * the weekly scoring job. Without this check that pick would silently keep
 * adding/subtracting a point in the standings total even though the game no
 * longer shows up on that week's public scoreboard - exactly the kind of
 * mismatch where the per-week grid looks right but the combined total
 * doesn't match a manual count.
 */
export function computeWeeklyPoints(
  ...
  const gamesById = new Map<string, Game>();
    for (const game of games) {
          if (!game.included_in_pickem) continue;
