 ...
 *
 * CFBD's `division: "fbs"` filter matches any game with an FBS participant,
 * which still includes "buy games" against FCS/lower-division opponents (the
 * away team in most of those). We only want true Division I FBS vs. FBS
 * matchups in the admin's selection list, so we additionally check each
 * team's classification (when CFBD provides it) and drop the game if either
 * side isn't "fbs". Older API responses that don't include a classification
 * field at all are kept as-is rather than guessed at.
 */
export async function getGames(year: number, week: number): Promise<CfbdGame[]> {
  ...
  return data
      .filter((raw: any) => {
              const homeClassification = raw.homeClassification ?? raw.home_classification ?? null;
              const awayClassification = raw.awayClassification ?? raw.away_classification ?? null;
              // If CFBD didn't return classification info for this game, don't
                    // exclude it just because we can't check - only filter when we have
                    // enough information to know for sure one side isn't FBS.
                    if (!homeClassification && !awayClassification) return true;
              const isFbs = (value: string | null) => !value || value.toLowerCase() === "fbs";
              return isFbs(homeClassification) && isFbs(awayClassification);
      })
